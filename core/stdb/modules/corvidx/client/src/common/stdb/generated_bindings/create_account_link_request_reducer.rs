...
impl From<CreateAccountLinkRequestArgs> for super::Reducer {
	fn from(args: CreateAccountLinkRequestArgs) -> Self {
		Self::CreateAccountLinkRequest {
			reference: args.reference,
		}
	}
}

impl __sdk::InModule for CreateAccountLinkRequestArgs {
	type Module = super::RemoteModule;
}

pub struct CreateAccountLinkRequestCallbackId(__sdk::CallbackId);

#[allow(non_camel_case_types)]
/// Extension trait for access to the reducer `create_account_link_request`.
///
/// Implemented for [`super::RemoteReducers`].
pub trait create_account_link_request {
	/// Request that the remote module invoke the reducer
	/// `create_account_link_request` to run as soon as possible.
	///
	...
	fn create_account_link_request(&self, reference: ExternalAccountReference)
	-> __sdk::Result<()>;
	...
	///
	/// The returned [`CreateAccountLinkRequestCallbackId`] can be passed to
	...
	/// future.
	fn remove_on_create_account_link_request(&self, callback: CreateAccountLinkRequestCallbackId);
}

impl create_account_link_request for super::RemoteReducers {
	fn create_account_link_request(
		&self, reference: ExternalAccountReference,
	) -> __sdk::Result<()> {
		...
		CreateAccountLinkRequestCallbackId(self.imp.on_reducer(
			...
			Box::new(move |ctx: &super::ReducerEventContext| {
				let super::ReducerEventContext {
					event:
						__sdk::ReducerEvent {
							reducer: super::Reducer::CreateAccountLinkRequest { reference },
							..
						},
					..
				} = ctx
	...
	fn remove_on_create_account_link_request(&self, callback: CreateAccountLinkRequestCallbackId) {
		self.imp
			.remove_on_reducer("create_account_link_request", callback.0)
	}
}
...
/// bump.
pub trait set_flags_for_create_account_link_request {
	/// Set the call-reducer flags for the reducer `create_account_link_request`
	/// to `flags`.
	///
	/// This type is currently unstable and may be removed without a major
	/// version bump.
	fn create_account_link_request(&self, flags: __ws::CallReducerFlags);
}

impl set_flags_for_create_account_link_request for super::SetReducerFlags {
	fn create_account_link_request(&self, flags: __ws::CallReducerFlags) {
		self.imp
			.set_call_reducer_flags("create_account_link_request", flags);
	}
}
