...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
	...
	_table.add_unique_constraint::<i128>("profile", |row| &row.profile);
}
...
/// Access to the `profile` unique index on the table `account`,
/// which allows point queries on the field of the same name
/// via the [`AccountProfileUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.account().profile().find(...)`.
pub struct AccountProfileUnique<'ctx> {
	imp:     __sdk::UniqueConstraintHandle<Account, i128>,
	phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> AccountTableHandle<'ctx> {
	/// Get a handle on the `profile` unique index on the table `account`.
	pub fn profile(&self) -> AccountProfileUnique<'ctx> {
		AccountProfileUnique {
			imp:     self.imp.get_unique_constraint::<i128>("profile"),
			phantom: std::marker::PhantomData,
		}
	}
}

impl<'ctx> AccountProfileUnique<'ctx> {
	/// Find the subscribed row whose `profile` column value is equal to
	/// `col_val`, if such a row is present in the client cache.
	pub fn find(&self, col_val: &i128) -> Option<Account> {
		self.imp.find(col_val)
	}
}
