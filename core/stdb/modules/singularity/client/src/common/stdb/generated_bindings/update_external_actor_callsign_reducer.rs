...
impl __sdk::InModule for UpdateExternalActorCallsignArgs {
	type Module = super::RemoteModule;
}

pub struct UpdateExternalActorCallsignCallbackId(__sdk::CallbackId);

#[allow(non_camel_case_types)]
/// Extension trait for access to the reducer `update_external_actor_callsign`.
///
/// Implemented for [`super::RemoteReducers`].
pub trait update_external_actor_callsign {
	/// Request that the remote module invoke the reducer
	/// `update_external_actor_callsign` to run as soon as possible.
	///
	...
	///
	/// The returned [`UpdateExternalActorCallsignCallbackId`] can be passed to
	/// [`Self::remove_on_update_external_actor_callsign`] to cancel the
	/// callback.
	fn on_update_external_actor_callsign(
		&self,
		callback: impl FnMut(&super::ReducerEventContext, &ExternalActorReference, &Option<String>)
		+ Send
		+ 'static,
	) -> UpdateExternalActorCallsignCallbackId;
	/// Cancel a callback previously registered by
	/// [`Self::on_update_external_actor_callsign`], causing it not to run in
	/// the future.
	fn remove_on_update_external_actor_callsign(
		&self, callback: UpdateExternalActorCallsignCallbackId,
	);
}
...
impl update_external_actor_callsign for super::RemoteReducers {
	...
	fn on_update_external_actor_callsign(
		&self,
		mut callback: impl FnMut(&super::ReducerEventContext, &ExternalActorReference, &Option<String>)
		+ Send
		+ 'static,
	) -> UpdateExternalActorCallsignCallbackId {
		UpdateExternalActorCallsignCallbackId(self.imp.on_reducer(
			"update_external_actor_callsign",
			Box::new(move |ctx: &super::ReducerEventContext| {
				...
				callback(ctx, ext_actor_ref, callsign)
			}),
		))
	}
	...
	fn remove_on_update_external_actor_callsign(
		...
		self.imp
			.remove_on_reducer("update_external_actor_callsign", callback.0)
	}
}
...
/// bump.
pub trait set_flags_for_update_external_actor_callsign {
	/// Set the call-reducer flags for the reducer
	/// `update_external_actor_callsign` to `flags`.
	///
	/// This type is currently unstable and may be removed without a major
	/// version bump.
	fn update_external_actor_callsign(&self, flags: __ws::CallReducerFlags);
}

impl set_flags_for_update_external_actor_callsign for super::SetReducerFlags {
	fn update_external_actor_callsign(&self, flags: __ws::CallReducerFlags) {
		self.imp
			.set_call_reducer_flags("update_external_actor_callsign", flags);
	}
}
